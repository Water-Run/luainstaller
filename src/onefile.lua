...
local function collectFiles(root, target_os)
    ...
    for _, item in ipairs(inventory) do
        ...
        if item.type == "file" then
            ...
            if rel ~= ".luai/generated-output.txt" then
...
static int luai_run_inner(const char *exe_path, int argc, char **argv) {
    ...
    if (luai_append_quoted(cmd, sizeof(cmd),
                           argc > 0 && argv[0] != NULL ? argv[0] : exe_path) != 0) {
...
#else
static int luai_run_inner(char *exe_path, int argc, char **argv) {
...
local function generateExtractorSource(inner_exe)
    local lines = {}
    lines[#lines + 1] = [[/* Generated by luainstaller.
 *
 * This is the exact extractor translation unit used for the onefile wrapper.
 * payload.inc is generated from the extracted payload files: sort normalized
 * safe relative paths bytewise; record path, executable mode, unsigned 64-bit
 * size, and exact bytes; then SHA-256 that concatenation for LUAI_PAYLOAD_ID.
 * The luainstaller source revision and RELINKING.adoc contain the generator.
 */]]
    ...
    lines[#lines + 1] = '#include "payload.inc"'
    lines[#lines + 1] = EXTRACTOR_TEMPLATE
    return table.concat(lines, "\n\n")
end

local function generatePayloadInclude(files, payload_id)
    local lines = {}
...
local function serializePayloadFiles(files)
    ...
    for _, file in ipairs(files) do
        lines[#lines + 1] = string.format(
            "    { path = %q, executable = %s },",
            file.path,
            file.executable and "true" or "false"
        )
    end
    lines[#lines + 1] = "}"
    lines[#lines + 1] = ""
...
function M.bundleOnefile(opts)
    ...
    local c_source = generateExtractorSource(inner_exe)
    local corresponding_c_path = normalizePath(
        stage_dir .. "/.luai/build/extractor.c"
    )
    err = ensureDirectory(dirname(corresponding_c_path))
    if not err then
        err = writeFile(corresponding_c_path, c_source)
    end
    if err then
        return cleanupDirectory(work_dir, err)
    end

    local payload_manifest_path = normalizePath(
        stage_dir .. "/.luai/build/payload-files.lua"
    )
    err = writeFile(payload_manifest_path, "-- generated by luainstaller\nreturn {}\n")
    if err then
        return cleanupDirectory(work_dir, err)
    end
    local provisional_files, provisional_err = collectFiles(stage_dir, profile.target_os)
    if not provisional_files then
        return cleanupDirectory(work_dir, provisional_err)
    end
    err = writeFile(payload_manifest_path, serializePayloadFiles(provisional_files))
    if err then
        return cleanupDirectory(work_dir, err)
    end
    ...
    err = writeFile(normalizePath(build_dir .. "/payload.inc"),
        generatePayloadInclude(files, payload_id))
    if err then
        return cleanupDirectory(work_dir, err)
    end
