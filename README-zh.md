...
需要强调的是, `luainstaller`保障的是打包后的二进制能够在和你相同的系统环境(抛去`lua`环境)下运行.  

> `luainstaller` 曾经以 Python 库的形式提供, 旧版本是开箱即用且跨平台的, 但仅能打包纯Lua脚本.(见`deprecated-python-lib`分支)  
...
> 在 Linux 上也可以用 `man luainstaller` 查看帮助  
