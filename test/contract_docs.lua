...
local function check_manifest_and_marker_contracts()
    ...
    assert_contains(marker, "luainstaller-generated-output-v1")
    ...
    for line in marker:gmatch("[^\n]+") do
        ...
        if kind == "file" and name == out:match("([^/]+)$") and #hash > 0 then
...
local function check_documentation_contract()
    local docs = command_output_trimmed("find docs -maxdepth 1 -type f -name '*.adoc' -printf '%f\\n' | sort")
    assert(docs == table.concat({
        "BUNDLING.adoc",
        "IMPLEMENTATION.adoc",
        "PLATFORMS-NATIVE-LIMITS.adoc",
        "TESTING.adoc",
        "TROUBLESHOOTING.adoc",
        "USAGE.adoc",
    }, "\n"), "docs/ must contain exactly the documented top-level guides")

    for name in docs:gmatch("[^\n]+") do
