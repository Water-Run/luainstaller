...
- [x] Keep current release docs explicit about Lua 5.4+ compatibility while
...
- [ ] Define Linux host, current Windows host, and Windows XP-compatible MinGW
      profiles.
- [x] Remove missing `executor.lua` and `wrapper.lua` from rockspec until they
      exist.
- [x] Register the CLI as `luai`.

### Phase 2: Public API And CLI Reset

- [x] Add primary CLI commands with `luai -a <entry>`, `luai -t <entry>`, and
      `luai -c <entry>`. Legacy compatibility aliases still exist.
- [x] Add `--onedir`, `--onefile`, `-o <path>`, `--include <path>`,
      `--exclude <path-or-module>`, `--no-depscan`, and `--verbose`.
- [x] Make `--onedir` the default output mode.
- [x] Expose `luainstaller.analyze(opts)`, `luainstaller.trace(opts)`, and
      `luainstaller.bundle(opts)`.
- [x] Return structured API results.

### Phase 3: Traceable Analyzer

- [x] Keep the existing lexer and resolver.
- [x] Add trace records with requiring file, source line, requested module,
      candidate templates, selected path, selected type, and skip/failure
      reason.
- [x] Classify modules as `builtin`, `lua`, `native`, `missing`, or `excluded`;
      `external` remains reserved for explicit external library work.
- [x] Treat `pcall(require, "...")` as optional in trace output.
- [x] Preserve dependency-limit and circular-dependency errors.

### Phase 4: Bundle Manifest

- [x] Create `src/manifest.lua`.
- [x] Convert analyzer output, includes, excludes, and output options into a
      manifest table.
- [x] Store Lua version, Lua ABI note, OS, architecture, output mode, launcher
      profile, and content hashes.
- [x] Use normalized forward-slash paths inside the manifest.
- [x] Reject ambiguous duplicate module names unless explicitly resolved.

### Phase 5: Pure Lua Runtime Searcher

- [x] Create `src/runtime.lua` and `src/cgen.lua`.
- [x] Generate a Lua bootstrap chunk that installs a bundled searcher.
- [x] Support Lua source chunks by module name and entry id.
- [x] Strip UTF-8 BOM and shebang before loading source.
- [x] Preserve `arg` semantics.
- [x] Verify single-file and multi-file pure Lua samples.

### Phase 6: C Launcher Template

- [x] Add `src/launcher/luai_launcher.c`.
- [x] Add platform helpers for POSIX and Windows.
...
- [x] Embed Lua payload according to manifest-derived dependency planning.
...
- [x] Embed staged onedir payload files as generated C byte arrays.
- [x] Teach the outer extractor to write and validate payload files by size and
      hash.
- [x] Extract native modules before user code.
- [x] Use two-stage extraction on Windows when `lua54.dll` is required.

### Phase 10: Extraction And Cleanup

- [x] Create content-addressed extraction cache directories.
- [ ] Write native modules atomically where possible.
- [x] Reuse extraction only when hashes match.
- [ ] Clean temp directories on normal exit.
- [x] Leave readable diagnostics when extraction fails.
- [x] Verify repeated onefile runs do not use stale modules.

### Phase 11: Compatibility Diagnostics

- [x] Create `src/compat.lua`.
- [x] Record Lua version and Lua ABI notes in every manifest.
- [x] Record launcher profile notes for host exports, `lua54.dll`, shared Lua
      library, or static registration where the current profile makes that
      explicit.
- [x] Detect obvious target profile versus host OS differences.
- [x] Warn about likely external-library dependencies.
- [x] Add a `luai -t` section explaining same OS, architecture, and ABI.
