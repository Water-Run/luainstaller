...
Without LuaRocks, run directly from an extracted source tree:
...
Or install offline into a new dedicated directory (its parent must exist):
...
Windows uses the same Lua installer and generates `.cmd` launchers. Source
library use, shell setup, relocation and prerequisites are documented in
xref:docs/INSTALL.adoc[Installation without LuaRocks]. Building applications
still requires a native C compiler and matching Lua headers/library.
...
v1.3 selects implementations by native capability rather than by a short OS
allowlist. Builds remain native: the compiler, Lua headers/library, native
modules, architecture and target OS must agree. The release gates and
platform-specific contract cover:
...
Windows XP builds require PowerShell 2 or newer and an XP-targeting compiler
runtime; installing a modern compiler on a new host does not by itself make
its CRT XP-compatible. Full requirements and evidence tiers:
xref:docs/PLATFORMS-NATIVE-LIMITS.adoc[Platforms and native modules] and
xref:docs/TESTING.adoc[Testing].
